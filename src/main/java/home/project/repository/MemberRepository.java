...
public interface MemberRepository extends JpaRepository<Member, Long> {
    ...
    @Query("SELECT m FROM Member m JOIN m.role r WHERE " +
            ...
            "(:role IS NULL OR :role = '' OR LOWER(r.role) LIKE LOWER(CONCAT('%', :role, '%'))) AND " +
            "(:content IS NULL OR :content = '' OR " +
            "LOWER(CONCAT(m.name, ' ', m.email, ' ', m.phone, ' ', r.role)) LIKE LOWER(CONCAT('%', :content, '%')))")
